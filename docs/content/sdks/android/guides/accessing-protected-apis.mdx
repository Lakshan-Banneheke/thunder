...
suspend fun fetchProtectedResource(thunder: ThunderIDState): String {
    ...
    return withContext(Dispatchers.IO) {
        val connection = URL("{{ServerUrl}}/api/resource").openConnection() as HttpURLConnection
...
:::tip API Server URL
The <ProductName /> API server runs on `{{ServerUrl}}` by default in local development. Replace this with your production server URL when deploying.
:::
