...
export async function fetchUserProfile(sessionId: string) {
  ...
  const response = await fetch('{{ServerUrl}}/users/<user_id>', {
    headers: {
...
export async function fetchDashboardData(sessionId: string) {
  ...
  const [profile, organizations] = await Promise.all([
    fetch('{{ServerUrl}}/users/<user_id>', { headers }).then(r => r.json()),
    fetch('{{ServerUrl}}/organizations', { headers }).then(r => r.json()),
  ])
