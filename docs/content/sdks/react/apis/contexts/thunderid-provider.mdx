...
docType: reference
description: Root context provider that wraps your React app and supplies authentication state to all child components.
---
...
The `<ProductName />Provider` initializes the <ProductName /> authentication client, manages authentication state, and provides context to child components through React Context. It handles token management, user sessions, organization switching, and branding preferences automatically.
...
| `baseUrl` | `string` | ✅ | The base URL of the <ProductName /> instance (e.g., `{{ServerUrl}}`) |
