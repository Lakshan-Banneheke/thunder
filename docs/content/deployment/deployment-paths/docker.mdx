...
```bash
docker pull ghcr.io/thunder-id/thunderid:<version>
```
...
<ProductName /> is now running at `{{ServerUrl}}`. Mount the same `thunderid-certs` and `thunderid-secrets` volumes you used during setup so the server reads the generated key material and the Direct Auth Secret.
...
| <ProductName /> Console | `{{ConsoleUrl}}` |
