...
3. **Integrate with your application**:
   - Redirect users to <ProductName />'s OAuth2 authorize endpoint:
     ```text
     {{ServerUrl}}/oauth2/authorize?client_id=<your-client-id>&redirect_uri=<your-callback>&response_type=code&scope=openid
     ```
...
```bash
curl -k -X POST {{ServerUrl}}/register/passkey/start \
...
```bash
curl -k -X POST {{ServerUrl}}/register/passkey/finish \
  -H "Content-Type: application/json" \
  -d '{
    "publicKeyCredential": {
      "id": "<credential-id>",
...
```bash
curl -k -X POST {{ServerUrl}}/auth/passkey/start \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -k -X POST {{ServerUrl}}/auth/passkey/finish \
  -H "Content-Type: application/json" \
  -d '{
    "publicKeyCredential": {
      "id": "<credential-id>",
...
```bash
curl -k -X POST {{ServerUrl}}/flow/execute \
  -H "Content-Type: application/json" \
  -d '{
