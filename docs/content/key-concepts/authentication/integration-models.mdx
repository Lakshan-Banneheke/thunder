...
```bash
curl -X POST {{ServerUrl}}/auth/credentials/authenticate \
...
```bash
curl -X POST {{ServerUrl}}/auth/otp/sms/send \
  -H 'Content-Type: application/json' \
  -d '{
    "senderId": "<sender_id>",
...
```bash
curl -X POST {{ServerUrl}}/auth/otp/sms/verify \
  -H 'Content-Type: application/json' \
  -d '{
