...
An agent can hold an access token that represents a user instead of itself. The token names the user in the `sub` claim, and on the sign-in and exchange paths it also names the agent in the `act` (actor) claim, so a resource sees both the user who authorized the action and the agent that carried it out. <ProductName /> issues these tokens once the agent is extended to act on behalf of a user.
...
```bash
curl -X POST {{ServerUrl}}/oauth2/token \
...
- With no `resource` and no permission scopes, the token is not bound to a resource server: its audience falls back to the agent's default audience, or its client ID when no default audience is set, and only OIDC scopes such as `openid` remain.
