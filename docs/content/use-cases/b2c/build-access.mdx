...
```bash
curl -k -X POST {{ServerUrl}}/resource-servers \
...
```bash
curl -k -X POST {{ServerUrl}}/resource-servers/<wayfinder-rs-id>/resources \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
for action in "Read:read" "Create:create" "Cancel:cancel"; do
  curl -k -X POST {{ServerUrl}}/resource-servers/<wayfinder-rs-id>/resources/<booking-resource-id>/actions \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
