...
Every API call below needs an access token with the `system` scope. Obtain one for an administrator and export it, so the calls that follow can reuse it:

```bash
export TOKEN="<access-token>"
```

Each call then sends `-H "Authorization: Bearer $TOKEN"`.
...
1. **Create the `wayfinder-agent` resource server.**
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/resource-servers \
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/resource-servers/<wayfinder-agent-rs-id>/resources \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/resource-servers/<wayfinder-agent-rs-id>/resources/<agent-resource-id>/actions \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
...
2. **Create the `wayfinder` resource server.**
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/resource-servers/<wayfinder-rs-id>/resources \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/resource-servers/<wayfinder-rs-id>/resources/<booking-resource-id>/actions \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
   ...
   ```bash
   curl -kL -X PUT {{ServerUrl}}/server-config/defaultResourceServer \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
...
3. **Create the `Chat User` role.**
   ...
   ```bash
   curl -kL -X POST {{ServerUrl}}/roles \
     -H 'Content-Type: application/json' \
     -H "Authorization: Bearer $TOKEN" \
     -d '{
       "name": "Chat User",
...
1. **Add the agent.**
   ...
   - On the **Advanced** tab, under **Operating Mode**, turn on **Delegated mode**. That adds the `authorization_code` grant the on-behalf-of flow needs. Client credentials stays on either way.
   - Still on **Advanced**, add the redirect URI `{{WayFinderSampleUrl}}/agent-callback`.

   See [Create and Manage Your Agent](../../../guides/agents/manage-agents) and [Act on Behalf of a User](../../../guides/agents/authentication/on-behalf-of-user).
...
Attach the flow to the `WAYFINDER-CONCIERGE` agent: set `authFlowHandle` in a declarative file, or `authFlowId` through the management API.
