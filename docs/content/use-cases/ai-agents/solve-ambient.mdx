...
sequenceDiagram
  ...
  participant TID as {{ProductName}}
  ...
  TID-->>Agent: Issues the token, naming the person and the agent.
```
...
What comes back is a delegated token like any other: it names the person as the subject and the agent as the
actor, so the service that receives it applies their permissions and records which agent acted. The difference
is in how the approval was collected, not in what the token carries.
