...
The MCP server is available through the `/mcp` endpoint of your <ProductName /> instance (default: `{{ServerUrl}}/mcp`). MCP clients discover the available tools automatically through the protocol.
...
- <ProductName /> server running (default: `{{ServerUrl}}`)
...
MCP Inspector's proxy runs on Node.js. Point Node.js at the <ProductName /> self-signed certificate so TLS connections to `{{ServerUrl}}` are trusted, without adding anything to your system trust store:
...
Claude Code runs on Node.js. Point it at the <ProductName /> self-signed certificate so TLS connections to `{{ServerUrl}}` are trusted, without adding anything to your system trust store:
...
```bash
claude mcp add --transport http thunderid-mcp {{ServerUrl}}/mcp
```
