...
:::tip Success
Access <ProductName /> on **<ConsoleUrl plain />**
:::
...
helm install {{productSlug}} oci://ghcr.io/thunder-id/helm-charts/{{productSlug}} \
  ...
  --set configuration.server.publicUrl={{ServerUrl}} \
...
Open `{{ConsoleUrl}}` and sign in using the admin username with that password.
