...
```bash title=".env"
THUNDERID_ISSUER={{ServerUrl}}
THUNDERID_JWKS_URI={{ServerUrl}}/oauth2/jwks
...
Both the issuer and the JWKS URI are discoverable from <ServerUrl /> if you're pointing at a non-default host or port.
...
```bash
curl -sk -i -X POST {{ServerUrl}}/oauth2/dcr/register \
