...
```bash title=".env"
# {{ProductName}} (local, self-signed cert)
THUNDERID_BASE_URL=https://localhost:8090

# The agent's OAuth client credentials (from the {{ProductName}} Console)
...
# --- {{ProductName}} agent authentication (client_credentials) ---
def get_agent_token() -> str:
    """Fetch a brand new client_credentials token. No caching, no reuse."""
    data = {"grant_type": "client_credentials"}
    if SCOPE:
...
# --- {{ProductName}} OBO authentication (authorization_code + PKCE) ---
